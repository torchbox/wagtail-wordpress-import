...
@override_settings(
    WORDPRESS_IMPORT_HOOKS_TAGS_TO_CACHE={
        "wp:foo": {"DATA_TAG": "dc:creator", "FUNCTION": "path.to.function"},
        "wp:bar": {"DATA_TAG": "dc:creator", "FUNCTION": "path.to.function"},
    }
)
class WordpressImporterTestsCheckXmlTagsCached(TestCase):
    """
    ...
    @mock.patch.object(TagsCache, "process", process)
    def process_import(self, xml_stream):
        self.importer = WordpressImporter(xml_stream)
        self.logger = Logger(LOG_DIR)
        self.importer.run(
    ...
    def test_xml_tags_cached(self):
        ...
        <item>
            ...
            <link>https://www.example.com/a-title</link>
            <description />
            <dc:creator>some data</dc:creator>
            <content:encoded />
            <excerpt:encoded />
            <wp:post_id>44221</wp:post_id>
            <wp:post_date>2015-05-21 15:00:31</wp:post_date>
...
def foo_handler(page, data, tags_cache):
    return page, data, tags_cache
...
@override_settings(
    WORDPRESS_IMPORT_HOOKS_TAGS_TO_CACHE={
        "wp:author": {
            "DATA_TAG": "dc:creator",
            "FUNCTION": "wagtail_wordpress_import.test.tests.test_import_hooks_tags.foo_handler",
        },
    }
)
class TestImportHooksTagsCacheMethods(TestCase):
    ...
    def test_get_hook_func_data(self):
        ...
        self.process_import(built_file)
        function, actions = self.importer.tags_cache._get_hook_handler_data()
        page, data, tags_cache = import_string(function)(
            self.imported_pages[0], actions, self.importer.tags_cache.__dict__
        )

        self.assertIsInstance(page, Page)
        self.assertEqual(page.title, "A title")
        self.assertIsInstance(data, str)
        self.assertEqual(data, "dc:creator")
        self.assertIsInstance(tags_cache, dict)
        self.assertEqual(tags_cache["wp:author"][0]["wp:foo_login"], "rickw")
